#!/usr/bin/env node
/**
 * Encoding pre-flight for dsh-git-bash.
 *
 * The documented way to brick a dsh profile is a UTF-8 BOM: the loader
 * `JSON.parse`s documents directly, so `EF BB BF` turns a plugin that "worked
 * yesterday" into a host that refuses to start. This sweeps every shipped text
 * file for that byte order mark, for replacement characters (a file decoded
 * with the wrong code page and re-saved), for tabs used as YAML indentation,
 * and for JSON that no longer parses.
 *
...
for (const absolute of files) {
  ...
  if (rel === 'cordis.patch.yml') {
    ...
    if (!/^\s+- id: dsh-git-bash$/m.test(text)) {
      fail('cordis.patch.yml must insert the row id "dsh-git-bash"')
    }
    if (!/^\s+name: 'dsh-git-bash'$/m.test(text)) {
      fail('cordis.patch.yml must insert the row name "dsh-git-bash"')
    }
  }
}

// The two files the loader parses before any of this plugin's code runs.
